...
- **Kolejność obchodu (zmiana 2026-07-24, rozszerzona 2026-07-30 i 2026-08-04):** domyślnie po kodzie lokalizacji (kolejność zbierania), ale **kilka grup idzie na KONIEC**, rosnąco: `zwykłe < poprezentacyjne/uszkodzone (p/u) < w strefie < w pełni zarezerwowane < pominięte < policzone niedawno`. Wewnątrz każdej grupy nadal po lokalizacji. Czysta logika w `services/kolejnosc-obchodu.js` (`grupaObchodu`/`porownajObchod`, testy `test/kolejnosc-obchodu.test.js` bez SQLite/GT); sort w `/ostatnie-sztuki` **po** `dolaczOczekiwanaPolke` (potrzebuje `w_strefach`). Powód: te rodzaje wymagają innej uwagi niż liczenie pełnej półki (p/u = egzemplarz specjalny; strefa = część stanu leży poza półką; pełna rez = z półki i tak nic nie ruszysz), więc nie mają rozbijać rytmu obchodu zwykłych pozycji. Grupy z IF-ów w kolejności „najdalsza cecha wygrywa" — pozycja i w strefie, i zarezerwowana ląduje w rezerwacji; policzone niedawno bije **wszystko**.
  ...
  - **„Pominięte" (grupa 4) — pominięcie NIE usuwa pozycji ze spisu (zmiana 2026-08-04, zgłoszenie usera).** Wcześniej „Pomiń" chował parę (artykuł+lokalizacja) na 7 dni: zadanie znikało z listy, a po tygodniu wracało **na swoje normalne miejsce**, czyli zwykle na początek obchodu — przed wszystkim, czego nikt jeszcze nie liczył (a przy ~700 pozycjach nikt nie przechodzi listy w jednym cyklu, więc magazynier co obchód dostawał tę samą blokadę na starcie). Teraz pominięta pozycja **zostaje na liście, tylko na jej końcu**: w bieżącym obchodzie front przerzuca ją na koniec listy (`pominPrzystanek` w `sciezki.js`; drugie pominięcie już jej nie cofa — inaczej „pomiń wszystko" kręciłoby listę w kółko), a między obchodami robi to grupa sortu. Okno `DNI_KONIEC_POMINIETE` = `DNI_POMIN_SPRAWDZONE` (180 dni) — skoro pozycja wciąż jest na liście, to znaczy, że **nikt jej nie policzył**, więc siedzi na końcu aż do policzenia. Pozostałe ścieżki (K4 rezerwacja, Czyść zera, Brak parametrów) mają to samo zachowanie przez `naKoniecPominiete` (stabilna partycja po sorcie) — sortują po jednym kryterium, więc nie potrzebują grup.
...
- Wyklucza parę (artykuł+lokalizacja) sprawdzoną w ciągu **180 dni** (`DNI_POMIN_SPRAWDZONE`) — **pominięcie NIE jest wykluczeniem**, patrz grupa „Pominięte" wyżej — oraz SKU z przyjęciem z magazynu zewnętrznego (`ruchy.mag_zrodlo_zewnetrzny` NOT NULL) w ciągu **30 dni** (`DNI_POMIN_PRZYJECIE`) — świeżo dołożony stan jest znany. Oba filtry z SQLite, w Node.
