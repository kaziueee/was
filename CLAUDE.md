...
Statusy liczy czysty `services/zajetosc-model.js` (testy `test/zajetosc-model.test.js`, bez SQLite i GT): `zajeta` → `pusta_polka` („Wolne do sprawdzenia") → `tylko_gt` → `wolna` / `nietknieta`. Kolejność zakładek na ekranie jest inna niż kolejność rozstrzygania — idzie od „na pewno zajęte" do „na pewno wolne" (`Zajęte · Tylko GT · Wolne do sprawdzenia · Wolne · Nigdy nietknięte`) i bierze się z `OPISY_STATUSOW`. Kolejność warunków = **prawda WMS przed domysłem z GT**: wiersz w `stany_lokalizacji` zawsze wygrywa (zasada #2), GT rozstrzyga tylko tam, gdzie WMS milczy. `wolna` vs `nietknieta` = czy slot w ogóle występuje w `ruchy`/audycie. Zbieranie danych (SQLite + jeden przebieg po kartotece GT) w `services/zajetosc.js`, endpoint `GET /api/lokalizacje/zajetosc`.
