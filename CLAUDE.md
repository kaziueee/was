...
**Kiedy pola są przeliczane — magazyny ruchu + strażnik deficytu (2026-08-13).** `synchronizujLokalizacje` dostaje ZBIÓR magazynów dotkniętych ruchem i tylko dla nich rusza pole. Zbiór składa `magazynyRuchu` w `services/ruchy-model.js` (czyste, testowalne) i liczy się w nim także **`mag_zrodlo_pula`**: rozłożenie (`POST /ruchy/rozloz`) nie ma lokalizacji źródłowej, bo źródłem jest **nieprzypisana pula magazynu** („wg GT towar leży na K4, WMS nie wie gdzie"). Pula wchodzi do zbioru **tylko gdy WMS ma w tym magazynie jakikolwiek wiersz** — inaczej wyliczone pole byłoby puste, a pusty string kasuje pole w GT, czyli skasowałby adres istniejący WYŁĄCZNIE w GT (ręczne `RB/A18 /`), którego ten ruch w ogóle nie dotyczył.

Osobno działa **strażnik deficytu**: dopóki cały stan GT nie jest rozłożony w WMS, `tw_Pole1`/`tw_Pole8` NIE są nadpisywane (WMS ma cząstkowy obraz; przy K4 chroni to zapas trzymany tylko w GT, np. `A1/P5`). **Wyjątek dla `tw_Pole1`: gdy pole w GT jest PUSTE, a WMS zna dom — piszemy mimo deficytu.** Strażnik chroni treść, a przy pustym polu nie ma czego chronić. Puste znaczy też „sam dopisek stref" (`+StD48`) — to nie adres.

Bez tych dwóch reguł rozłożenie jednego towaru na K4 i K4G **w jednym podejściu** zostawiało `tw_Pole1` puste na zawsze (zgłoszenie MATJFG56-JCR38): LOK 40 szt. z puli K4 na półkę → w GT leży jeszcze 48, deficyt 8, pole pominięte; MM 8 szt. z tej samej puli na K4G → deficyt spada do 0, ale magazyny = `{K4G}`, bo źródłem była pula. WMS miał dom `M2-J14`, GT puste pole, karta produktu świeciła **NZ mimo poprawnie rozłożonego towaru**. Odwrotna kolejność (najpierw K4G) działała — stąd „czasem się zdarza". Pominięta synchronizacja **nie ma automatycznej dosyłki**: jeśli reszta stanu zejdzie sprzedażą, deficyt zamknie się sam poza WMS i nic już nie odpali zapisu — dlatego wyjątek „puste pole" jest ważniejszy niż wygląda. Zaległości sprząta `node scripts/backfill-tw-pole1.js` (dry-run; `--zapisz` uzupełnia WYŁĄCZNIE puste pola, rozjechaną treść tylko wypisuje).
...
| **Zestaw (rodzaj 8) nie dostaje lokalizacji WMS** — leżą jego składniki, nie on | ✅ backend | `/ruchy/lok`, `/mm` (cel WMS), `/przyjecie`, `/rozloz` |
...
**„Zestaw nie ma lokalizacji" — status `ZEST` i strażnik przy zapisie (2026-09-22).** Komplet (`tw_Rodzaj = 8`) nie leży na półce jako paczka: jego stan to zapis księgowy, a fizycznie są to składniki, każdy na swoim miejscu (`SPIN20148990Z1` = 12 szt. w GT = 48 lalek na czterech różnych półkach). Wiersz WMS na zestawie jest więc **zawsze nieprawdą o miejscu** — i wpisuje tę nieprawdę do `tw_Pole1`, czyli do jedynego pola, które widzi człowiek szukający towaru w Subiekcie.

- **Werdykt `ZGODNOSC.ZESTAW` = `ZEST` („nie dotyczy"), bez wyjątków.** Wcześniej zestawy dostawały `BD` albo `t_GT`, czyli zadanie, którego nikt nigdy nie wykona: na produkcji **67 zestawów** siedziało w uniwersum Zgodności (42 w `BD`, 25 w `t_GT`), przez co filtr `BD` pokazywał 64 pozycje, z których prawdziwych było ~20. Rozstrzyga `tw_Rodzaj` z `vwPolaWlasne_Towar` (widok ma wiersz dla każdego towaru: 94588 = `COUNT(tw__Towar)`), a werdykt jest **terminalny** — stawiany przed jakimkolwiek porównaniem. Tekst z GT zostaje do POKAZANIA: ~90 kompletów ma tam ręcznie wpisany adres składników (`RB`, `M2-B36-P1`, `A17 | B9P2/B4P2 /`) i to notatka człowieka, której nie kasujemy ani nie sprawdzamy.
- **Pierwotna wersja tej reguły miała wyjątek „chyba że WMS zna miejsce"** — oparty na 4 wierszach uznanych za multipaki leżące na półce. Okazały się pomyłkami przypisania (patrz niżej), więc wyjątek odpadł. Zestaw to `ZEST` zawsze.
- **Strażnik siedzi w backendzie, nie na ekranach.** Regułę znali ludzie, nie kod — i w 5 tygodni została złamana 5 razy (19.08 → 21.09, zawsze przez „przypisanie"). Jeden wiersz w WMS otwiera zestaw na **cztery ekrany naraz**: Zgodność, „Do zlokalizowania", obchód „Ostatnie sztuki" (gałąź (b) bierze SKU, które WMS zna, bez filtra rodzaju) i zajętość. Dlatego blokada stoi przy zapisie, a nie przy każdym wyświetlaniu (zasada 5).
- **Granica jest jednokierunkowa: blokujemy WEJŚCIE na lokalizację, nie wyjście.** `/ruchy/mm` z celem zewnętrznym (BRK, K4R) zostaje dozwolone — inaczej istniejące wiersze nie miałyby jak opuścić WMS. **Skutek uboczny: zestawu bez wiersza nie da się już wysłać na BRK** — dotąd jedyną drogą było przypisanie + MM (tak powstał `NERCHACU100` 21.09). Docelowo potrzebne MM z puli magazynu (K4/K4G → BRK/K4R bez lokalizacji źródłowej); do tego czasu taki dokument wystawia się ręcznie w Subiekcie.
- **GT niedostępne = przepuszczamy ruch** (`czyZestaw` zwraca `null`). Bilans błędów jest niesymetryczny: przepuszczony zestaw to jeden wiersz do skasowania, a zablokowany ruch przy padniętym GT zatrzymuje odkładanie całego magazynu. Tożsamość towaru nie jest inwariantem stanu — przed nadpisaniem stanów chronią osobne, twarde warunki (deficyt, rezerwacje), które i tak wymagają GT i same zwracają 503.
- **Sprzątanie istniejących wierszy idzie przez „Czyść zera"** — ta ścieżka nie filtruje po rodzaju, więc pusty zestaw pokazuje się na obchodzie jak każda pusta półka, a zwolnienie slotu **przelicza `tw_Pole1`**, czyli przy okazji kasuje fałszywy adres w Subiekcie.
...
2. **Kto ma `tw_Id`, nie wraca do systemu po symbolu.** Do tego są `/api/lokalizacje/artykul-id/:tw_Id` (desktop: rozjazdy, modal produktu, podpowiedzi lokalizacji) i `/api/lokalizacje/skan-id/:tw_Id` (Zebra: odświeżenie po zapisie, kontekst na Zwrotach, **każde wejście w ekran Ruch z listy**). Lookup po symbolu (`/artykul/:symbol`) zostaje dla wywołań, które mają sam kod — dziś nie woła go już nikt.
   **Front Zebry egzekwuje to w ośmiu miejscach (2026-09-23):** raport Ścieżek, „Do sprawdzenia", przywózki, przyjęcia wewnętrzne, dostawy, chipy „ostatnie produkty", tap w wynik wyszukiwania po nazwie i 🔍 przy zawartości lokalizacji. Wszystkie podawały SYMBOL do `wykonajSkan`, więc po przemianowaniu towaru w Subiekcie kończyły się „Nie znaleziono". Cztery z nich działały wyłącznie przypadkiem — brały symbol świeżo z GT; w raporcie Ścieżek tap był martwy **na zawsze**, bo symbol pochodzi z audytu, gdzie jest zamrożony celowo (pkt 6). `ruchOtworzArtykul(kod, {artykul_gt_id})` wchodzi po `tw_Id`, `kod` został etykietą.
3. **Potwierdzenie skanem rozstrzyga `tw_Id`, nie napis.** Obchód Ścieżek i wózek Zwrotów sprawdzają, czy magazynier trzyma właściwą sztukę — porównanie zeskanowanego kodu z symbolem/EAN-em **z listy**, a tam siedzi kopia kartoteki (w Zwrotach wręcz snapshot w `pozycje_wozka`, którego nie odświeża nawet job). Świeżo przemianowany towar nie zgadzał się więc z własną naklejką i ekran odrzucał dobrze wziętą sztukę. Po nietrafieniu napisów pytamy backend, co ten kod znaczy dziś (`czyTenSamTowar` w `kreator.js`) — **jedno zapytanie na ścieżce błędu**, nie na każdy skan. Brak sieci albo kod wieloznaczny (`lista_artykulow`) = dzisiejsza odmowa: potwierdzamy tylko jednoznaczną tożsamość.
...
5. **Przy kolizji nie zgadujemy.** Gdy jeden kod niesie w WMS >1 `tw_Id` (stary symbol przejęty przez inną kartę), `/skan` oddaje **listę do wyboru** (`kolizja_symbolu`), a `/artykul/:symbol` — 409. Dawne ciche `wiersze[0]` brało pierwszy alfabetycznie i doklejało mu cudze półki. Ta ścieżka odpala się tylko wtedy, gdy GT nie odpowiada albo nie zna kodu — przy żywym GT kolizji nie ma, bo tożsamość rozstrzyga `tw_Id`.
6. **`ruchy` i `audyt` zostają nietknięte** — tam symbol jest zapisem historycznym („tak nazywał się ten towar w chwili zdarzenia"). To dzięki niemu dało się odtworzyć ten incydent. Otwarte `rozjazdy` odświeżają się same: job czyta symbol z tej samej kopii.
...
- slot z **wierszem 0 szt.** na K4 jest pusty, ale nie wolny — to dom SKU czekający na uzupełnienie (inwariant „Lokalizacja K4 przeżywa stan 0"). Na ekranie nazywa się **„Wolne do sprawdzenia"** i stoi tuż przed „Wolne": nazwa ma mówić, co z tym zrobić (iść i sprawdzić, czy właściciel wraca — jeśli nie, zwolnić slot ścieżką „Czyść zera"), a nie tylko jak to wygląda. Do liczby wolnych **nadal się nie liczy**: tego miejsca nie dasz innemu towarowi, dopóki ktoś nie potwierdzi.

Statusy liczy czysty `services/zajetosc-model.js` (testy `test/zajetosc-model.test.js`, bez SQLite i GT): `zajeta` → `pusta_polka` („Wolne do sprawdzenia") → `tylko_gt` → `wolna` / `nietknieta`. Kolejność zakładek na ekranie jest inna niż kolejność rozstrzygania — zaczyna od tego, po co się tu przychodzi: `Wolne · Wolne do sprawdzenia · Zajęte · Tylko GT · Nigdy nietknięte` (z `OPISY_STATUSOW`). Kolejność warunków = **prawda WMS przed domysłem z GT**: wiersz w `stany_lokalizacji` zawsze wygrywa (zasada #2), GT rozstrzyga tylko tam, gdzie WMS milczy. `wolna` vs `nietknieta` = czy slot w ogóle występuje w `ruchy`/audycie. Zbieranie danych (SQLite + jeden przebieg po kartotece GT) w `services/zajetosc.js`, endpoint `GET /api/lokalizacje/zajetosc`.
...
**Wolne miejsce w rozbiciu na RODZAJE (2026-09-18).** Sama liczba „wolnych" nie mówi, czy zmieści się paleta, więc kafle — i na pulpicie, i na ekranie — pokazują **trzy rozłączne kubełki**: `K4 Góra` (miejsce paletowe na górze) · `K4 półka` (półki regałowe na dole, drobnica) · `K4` (reszta dołu: palety, trawersy, miejsca nazwane). Podział idzie po `typ`, który i tak jest wyliczany z kodu — bez nowej kolumny; **`K4 półka` musi stać przed `K4`**, bo wygrywa pierwsze dopasowanie (ten sam wzorzec, co `grupaObchodu`). Definicje w `GRUPY_MIEJSC` (`services/zajetosc-model.js`), liczy `podsumujWolne`, a `services/zajetosc.js` dokleja `grupa` do każdej pozycji, żeby front nie powtarzał reguły u siebie. Kafel klika się w ekran z ustawionym filtrem rodzaju.

**Kafel liczy `wolna` + `nietknieta` razem** — oba znaczą „tu da się coś położyć", różnią się wyłącznie PEWNOŚCIĄ (czy WMS kiedykolwiek ten slot widział). Nagłówkowa liczba je łączy, a ilu z nich nikt nigdy nie tknął, stoi w podpisie kafla i w zakładce „Nigdy nietknięte" (ta sama liczba — bez tego kafel i zakładka mówiłyby co innego o tym samym).

- **Kafel na pulpicie idzie ze snapshotu** (godzinny job `services/pulpit-snapshot.js`, klucz `zajetosc`), więc pokazuje te same, zweryfikowane w GT liczby co ekran + „stan na HH:MM". Liczony samym WMS-em potrafił pokazać **„25% zajęte" tam, gdzie ekran mówił 0 wolnych** — bo wszystkie puste w WMS sloty miały towar opisany wyłącznie w polach GT. Nagłówkowa liczba ma znaczyć „ile miejsca jest zajęte", a nie „ile WMS o sobie wie", inaczej kafel kłamie dokładnie w tym przypadku, dla którego ten ekran powstał. Gdy snapshotu jeszcze nie ma (pierwsze minuty po starcie, padnięty Subiekt) backend schodzi na rachunek z samej `wms.db` i oznacza go `zrodlo:'wms'` — kafel podpisuje się wtedy „wolne wg WMS … · bez weryfikacji w GT", zamiast podawać niesprawdzoną liczbę jako pewną.
...
1. **Ruch towaru** (`ruch.html`) — zlany MM + zmiana lokalizacji. Skan SKU/EAN/lokalizacji → wybór → krok „Dokąd i ile?": select **Cel** (Ta sama = LOK w obrębie magazynu / inny magazyn = MM) + ilość + lokalizacja. Operacja LOK/MM wyprowadzana automatycznie. Po zatwierdzeniu ekran sukcesu (dotknięcie zamyka) + sygnał dźwiękowy.
   ...
   - **Wiersz zawartości lokalizacji ma DWA cele dotykowe (2026-08-19):** treść = „przenieś stąd" (jak dotąd), **🔍 z prawej = karta produktu** — ten sam rozkład co po skanie SKU (inne lokalizacje towaru, rezerwacje na K4, zestawy, stan Razem). Wcześniej z zawartości lokalizacji jedyna droga dalej prowadziła prosto w „Dokąd i ile?", więc towaru **nie dało się zbadać bez ruszania go** — trzeba było wyjść i zeskanować SKU osobno. Wstecz z karty wraca do zawartości lokalizacji (`powrotDoLokalizacji`, ta sama ścieżka co dla wierszy `t_GT`) i **doczytuje ją na nowo** (`wrocDoLokalizacji`): kopia w pamięci kłamie po ruchu, bo pętla „➕ Dalej" nie resetuje kreatora. Kopia rysuje się od razu (ekran nie miga pustką i przeżywa brak sieci), świeża lista podmienia ją, gdy odpowiedź dojdzie — chyba że magazynier zdążył już gdzieś wejść: `widocznaLokalizacja` (marker „co JEST teraz na ekranie", zerowany w `przygotujKrokWybor`, więc każdy nowy tryb kroku „wybór" kasuje go sam) plus licznik zadania pilnują, żeby spóźniona odpowiedź nie nadpisała cudzego widoku. Pusto po doczytaniu = czyścimy listę i mówimy to wprost, zamiast pokazywać nieaktualną.
...
4. **Sprawdź** (`sprawdz.js`, widok `#widok-sprawdz` w `ruch.html`) — sprawdzarka: „co tu leży" (skan lokalizacji) i „gdzie to leży" (skan SKU/EAN, wpisana nazwa). **Czysty podgląd — żadnego zapisu, żadnego pola ilości, żadnego wejścia w Ruch.** Tap w pozycję otwiera kolejny podgląd, więc ekran nie ma ani jednej ścieżki kończącej się zmianą stanu.
   - **Po co osobny ekran, skoro Ruch pokazuje to samo:** żeby pytanie „gdzie jest ten towar" dało się zadać bez wchodzenia na ekran, który potrafi wystawić MM. Uczeń Ruchu nie widzi, więc nie miał jak sprawdzić półki, choć to czynność bez ryzyka; a każde „tylko zobaczyć" kończyło się w kreatorze z polem ilości i zajętym lockiem edycji (`BlokadaEdycji`) — towar wyglądał na zajęty przez kogoś, kto tylko patrzył.
   - **Dostępna dla KAŻDEJ roli, w tym `uczen`** — kafel w `MENU_DLA_UCZNIA` (`ruch.js`), a zgodę **dubluje backend**: `blokujUcznia(['GET /kod', 'GET /skan', 'GET /skan-id'])` w `app.js`. Przepustki są **zawężone do GET** (`blokujUcznia` rozumie zapis `METODA /sciezka`), żeby dopisany kiedyś POST pod tym samym prefiksem nie wpadł uczniowi w pakiecie z odczytem — uprawnienie ma znaczyć „podgląd", nie „ta podścieżka". Widoczność kafla nie jest więc jedynym, co go tu wpuszcza (zasada 5).
   - **„Nie wiem" ≠ „zero".** Gdy GT nie odpowiada, `dolaczDaneGt` oddaje payload bez stanów — nie do odróżnienia od towaru, którego GT nie zna. Na ekranie do SPRAWDZANIA pewne „0 szt." dla towaru leżącego na półce jest najgorszą możliwą odpowiedzią, więc backend dokłada flagę **`gt_ok`** (`routes/lokalizacje.js`), a ekran pokazuje wtedy „—", podpis „stan wg GT nieznany" i pasek „GT nie odpowiada". Fakty z GT (stan, pola lokalizacyjne) **znikają**, miejsce w WMS zostaje — to własna wiedza WMS i akurat ona działa, gdy Subiekt leży.
   - **Układ karty produktu = rozkład jak w Ruchu (2026-09-25).** Główna treść to wiersze „gdzie leży i ile": kolorowa plakietka magazynu, **duży kod lokalizacji**, duża ilość i rezerwacja z boku — ten sam komponent (`.lista-poz`) i ta sama kolejność co rozkład źródeł na ekranie Ruch, żeby magazynier czytał jeden układ, nie dwa. Kolejność: **K4** (półka zbioru — pytanie numer jeden) → **K4G** → magazyny zewnętrzne → to, co leży **poza półką** (strefy). Surowy tekst pól GT i ostrzeżenia schodzą **pod** rozkład — to drobiazgi, nie odpowiedź. Wcześniej wszystko było listą „etykieta: wartość" i lokalizacja ginęła w tekście.
   - **Czego WMS nie zna, a GT ma na stanie, dostaje wiersz z kodem z pola GT** (podpis „wg pól GT — WMS nie zna tego miejsca"); dopiero gdy i tam pusto, wiersz robi się czerwony („nie wie ani WMS, ani GT"). **Gdy pole GT wskazuje półkę, którą WMS już zna, wiersze są SCALANE** — inaczej ten sam kod stał na ekranie dwa razy z dwiema różnymi liczbami (`M2-F28 — 1 szt.` i `M2-F28 — 136 szt.`). Scalony wiersz pokazuje stan wg GT, a w podpisie ile z tego WMS ma rozpisane. Porównanie kodów idzie „po gołej formie" (myślnik, `(n)`, człon `/zapas` ignorowane) — ta sama zasada, co `tokenyLokalizacjiZPola` po stronie backendu.
   - **Wstecz wraca na półkę, z której weszliśmy w produkt** (przycisk nosi wtedy jej kod), i **doczytuje ją na nowo** — kopia w pamięci kłamie, gdy ktoś w tym czasie coś stamtąd zdjął. Sprzętowy Back wychodzi od razu do menu i to jest zamierzone: podgląd jest bezstanowy, więc kolejne skany nie zasypują historii przeglądarki (inaczej wyjście po dziesięciu skanach = dziesięć tapów Back).
...
- `GET /api/zdrowie` oddaje **tylko zapamiętany stan** (`sfera: ok|odmowa|blad|nieznany`, komunikat, `zajety_od`, długość kolejki wątku STA). **Celowo nie woła Sfery** — health-check pukający w Sferę dokładałby zadań do tej samej zaklinowanej kolejki STA, czyli psułby to, co mierzy.
...
- **Odmowa biznesowa ≠ awaria mostu (2026-08-28).** Sfera potrafi odpowiedzieć merytorycznym „nie" (dziś: `0x80040F60` brak towaru na magazynie źródłowym) — dokument nie powstaje, ale GT, sesja i most działają, a MM innych towarów przechodzą. Most zwraca wtedy `rodzaj: "biznesowy"` w odpowiedzi `/api/mm`, zapisuje `StanPolaczenia.Odmowa` (→ `sfera: "odmowa"`, ikona w trayu zostaje **zielona**, w logu `ODMOWA` zamiast `BLAD`), a front zapala alarm wyłącznie na `stan === 'blad'` — więc kropka „Most" nie czerwienieje. Lista `BledyBiznesowe` w `SferaGtService.cs` jest **wąska celowo**: co nierozpoznane, zostaje techniczne, bo lepiej raz zapalić alarm za dużo niż przegapić awarię Sfery. Powód: 17 kolejnych odmów „Brak towaru" (patrz niżej) trzymało czerwony pasek w całym WMS przez pół dnia i wywołało trzy niepotrzebne restarty mostu.

## Kolejka ruchów — ponawianie, limit, duplikaty

Ruch zapisuje się jako `pending` i job (`services/ruchy-retry.js`, co 5 min) próbuje dogonić stronę GT. Statusy: `ok` · `pending` (w kolejce) · `wstrzymany` (job już nie ponawia, czeka na człowieka) · `duplikat` (to przesunięcie wykonał inny dokument). Czyste reguły w `services/ruchy-kolejka.js` (testy `test/ruchy-kolejka.test.js`, bez SQLite i GT).

**Limit ponawiania: 3 odmowy Sfery** (`LIMIT_ODMOW_SFERY`), potem `wstrzymany`. Liczy się **wyłącznie odpowiedź od Sfery** — licznik `ruchy.mm_odmowy` (osobny od `mm_proby`, które liczy każde wywołanie mostu). **Brak połączenia z mostem limitu nie zużywa**: most bywa wyłączony (restart peceta, aktualizacja) i wtedy ruch ma czekać dowolnie długo i dogonić się sam — to cały sens kolejki. Sfera, która odpowiedziała „nie", zdania sama nie zmieni. Ręczne „Ponów" na `wstrzymanym` zeruje licznik i wraca do kolejki („przyczyna usunięta, spróbuj jeszcze raz”). Wstrzymuje tylko odmowa **dokumentu** — zaległy sync pól lokalizacyjnych nie dotyka Sfery i nic nie kosztuje przy ponowieniu.

**Wykrywanie ręcznego powtórzenia.** Przy ponowieniu (`mm_proby > 0`), po nieudanym szukaniu **własnego** dokumentu (`znajdzMMpoKluczu`), WMS pyta jeszcze: czy magazynier nie zrobił tego samego przesunięcia drugi raz, ręcznie? `gt-dokumenty.znajdzPowtorzeniaMM` robi tani przesiew po stronie GT (typ MM, `dok_MagId` = magazyn źródłowy, ten sam towar, **ta sama ilość**, uwagi z kluczem `WMS-RUCH:` **innego** ruchu, od dnia ruchu), a decyzję podejmuje czysta reguła `dopasujPowtorzenie`: ten sam towar, ilość co do sztuki, ten sam kierunek, kandydat ma status `ok` i został zrobiony **po** naszym ruchu, w oknie 120 min. Trafienie → ruch dostaje `duplikat`, MM **nie jest wystawiane**. Dokument mówi tylko „to był ruch #N" — resztę dowodu (kierunek, status, czas) czytamy z WMS; podział jest celowy, żeby reguła „co jest duplikatem" była testowalna bez GT.

- **Stanów WMS przy powtórzeniu NIE cofamy automatycznie — nigdy.** Zamykamy sprawę (`duplikat`) tylko wtedy, gdy stany są uporządkowane **bez naszego udziału**, czyli gdy ruch miał **źródło w lokalizacji WMS** (`lok_zrodlo_id`): zdjął ilość z półki już przy zapisie, więc powtórka musiała zacząć od ponownego przypisania towaru (WMS nie przeniesie czegoś, czego na lokalizacji nie ma) — obie strony zeszły raz. **Ruch bez źródła w WMS** (rozłożenie z nieprzypisanej puli, przyjęcie z MAG/LS) dostaje tylko `wstrzymany`: tam powtórka mogła przejść bez żadnej kompensacji i dołożyć tę samą ilość drugi raz, a to już decyzja o stanach — należy do człowieka („Usuń" cofa, „Ponów" próbuje jeszcze raz). Reguła: `czyPowtorzenieZamykaRuch`.
- Gdyby wykrycie było fałszywe, w Logu zmian jest „Usuń", które cofa stany (`DELETE /api/ruchy/:id` przyjmuje `pending`, `wstrzymany` i `duplikat`, zawsze tylko bez `dok_gt_numer`).
- **`duplikat` nie wraca przez „Ponów"** (409) — ponowienie zrobiłoby MM drugi raz. Jedyne wyjście to „Usuń".
- Okno 120 min jest skończone celowo: powtórzenie to reakcja człowieka na komunikat błędu (w incydencie: 20 s), a ten sam MM nazajutrz to normalna praca magazynu.

**Skąd to się wzięło (NERG0319, 2026-08-28).** MM #5498 (15 szt., K4 `F2-P4` → BRK) odbił się od Sfery błędem `0x80040F1E` po 5,1 s (rzadka odmowa zapisu; w całym sierpniu 4 takie, wszystkie „długie" 3–8,5 s wobec typowych 300–500 ms, każda przechodziła przy ponowieniu). Magazynier nie czekał na job — powtórzył operację ręcznie: LOK #5499 + MM #5500 → `MM 1242/2026`, towar poszedł do BRK. Od tej chwili #5498 nie miał szans: job ponawiał go co 5 minut, Sfera odpowiadała „Brak towaru na magazynie źródłowym", a każda taka odmowa zapalała w całym WMS czerwony pasek „Sfera zgłosiła błąd". 17 prób, alarm bez końca, most i Sfera zdrowe, dane w GT poprawne (jeden dokument, `st_Stan` K4=0 / BRK=15). Bez limitu i bez wykrywania duplikatu taki ruch wisiałby w kolejce **na zawsze**.
