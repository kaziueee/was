...
- [x] 🟡 **Waga gabarytowa z kartonów + edytowalna lista kartonów w adminie** — ZROBIONE (commity `3914c38` + `93a5530`, 2026-07-23). Pole `pwd_Tekst10` „Waga gabarytowa karton DHL", edytowalna lista kartonów w panelu admina (tabela SQLite `kartony`, CRUD `/api/kartony`), separator kropka do GT (BaseLinker). Pełny opis w `CLAUDE.md` → sekcja „Waga gabarytowa z kartonu".
...
- [x] 🟡 **Backup lokalizacji + bazy do chmury** — ZROBIONE 2026-07-30 (kod). `services/backup-czytelne.js` (3 czytelne CSV: zbiorczy/płaski/historia, testy) + `services/backup.js` (paczka dnia `.db`+CSV → `rclone copy` jednokierunkowo do **Backblaze B2**, raz dziennie, retry+alarm). Włączane `WMS_RCLONE_REMOTE` (puste = wyłączone). Skrypt `scripts/backup-chmura-teraz.js` (1. uruchomienie + kwartalny test). Instrukcja: [docs/backup-chmura.md](docs/backup-chmura.md). **Zostało po stronie usera:** konto B2 + kubełek z Object Lock + klucz, `rclone` na pececie, zmienne w `.env` — patrz docs.
...
Waga z kartonów, desktop K4G w edycji i backup do chmury (kod) — ZROBIONE. Otwarte:

1. **🟡 Backup — dokończyć po stronie usera** — konto B2 + kubełek z Object Lock + klucz, `rclone` na pececie, `WMS_RCLONE_REMOTE` w `.env`, potem `node scripts/backup-chmura-teraz.js`. Instrukcja: [docs/backup-chmura.md](docs/backup-chmura.md).
2. **🟡 Kafle palet na pulpcie** — nowy agregat w `/api/pulpit` + klikalna lista lokalizacji (`typ=paleta`, stan 0).
3. **🔴 Decyzja o auto-czyszczeniu zer** — zanim cokolwiek kodować, rozstrzygnij tension z inwariantem (patrz sekcja 1).
