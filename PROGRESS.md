...
### 2026-07-02 — Tailscale: zdalny dostęp Mac → produkcja (Faza C#10, fundament)

Postawiony zdalny dostęp, żeby analiza/kontrola produkcji na Windowsie szła z Maca (Claude) z
dowolnej sieci — bez wystawiania portów na świat.

- **Sieć Tailscale** (konto `kaziueee@`): Mac `macbook-air-mati` = `100.84.80.55`,
  pecet `srv` (Windows) = `100.107.156.67`. Instalacja + login na obu (to samo konto).
- **Zweryfikowane z Maca:** `tailscale ping srv` → pong ~53ms (przez DERP fra — relay, może się
  podnieść do direct). Most osiągalny `http://100.107.156.67:5000` (404 na losowy endpoint = HTTP
  żyje), baza GT osiągalna TCP `100.107.156.67:49951`. Czyli zdalna analiza (SQL) i sterowanie
  (most) z Maca działa.
- **Rozdział ról potwierdzony:** git = wgrywanie kodu Mac→Windows (Node+most); Tailscale = dostęp
  sieciowy Mac→pecet (wgląd/kontrola). Nie zamiennik, uzupełnienie.
- **Do zrobienia:** ew. przełączyć konfig Node na Macu (`.env`: `GT_SQL_HOST`, `GT_BRIDGE_URL`) z
  LAN `192.168.0.200` na adres Tailscale (`srv` / `100.107.156.67`), żeby dev z Maca działał też
  spoza magazynowej LAN. Gdy Node przejdzie na Windows — dorobić `aktualizuj-node` (git pull+restart).
...
- **Konsola w tle** (dokończone tego samego dnia): most startuje jako sama ikona — konsola
  ...
  okno i most padł".
- **Autostart** ✅ (zrobione): skrót do `publish\GtBridge.exe` w folderze `shell:startup`
  (sesja użytkownika — Sfera COM tego wymaga, NIE usługa Windows). Most wstaje jako sama ikona po
  zalogowaniu. Niezawodność: most czyta `appsettings.json` też z katalogu `.exe`
  (`AppContext.BaseDirectory`, nie tylko CWD) — start z Autostartu/Harmonogramu nie gubi haseł.
- **Do zrobienia:** **Tailscale (dostęp zdalny) — przed uruchomieniem produkcyjnym** (patrz plan
  Faza C#10). **Gdy Node przejdzie na Windows:** autostart obejmie też Node, Tailscale da zdalny
  dostęp do całości, nasłuch mostu zawęzimy do `localhost`.
...
9. **Node + most na Windows.** Most: ✅ **ZROBIONE 2026-07-02** — ikona w trayu (status/restart/
   test/log), git clone na pececie, jedno-klik aktualizacja (`aktualizuj-most.cmd`), most bez okna
   konsoli, autostart (skrót w `shell:startup`). Zostaje: przepiąć **Node** (dziś na Macu) na Windows
   + autostart Node, i zawęzić nasłuch mostu z `0.0.0.0` do `localhost`, gdy Node stanie obok.
10. **Tailscale (VPN) — ✅ FUNDAMENT POSTAWIONY 2026-07-02.** Mac (`100.84.80.55`) i pecet
    „srv" (`100.107.156.67`) w jednej sieci (konto `kaziueee@`). Z Maca po Tailscale osiągalny
    most (`:5000`) i baza GT (`:49951`) — zdalna analiza/kontrola z Maca działa z dowolnej sieci
    (patrz Dziennik + [[tailscale-dostep-zdalny]]). Zostaje: wskazać konfig WMS na adres Tailscale
    (żeby Node↔most/GT działał też spoza LAN), gdy Node przejdzie na Windows.
11. **deploy.ps1 / rollback.ps1** (poprawki z Maca: backup→build→health-check→auto-rollback).
