...
// Czysta logika rozbicia stanu + rejestr rodzajow stref zyja w osobnym module BEZ zaleznosci
// od SQLite/GT, zeby dalo sie je testowac bez otwierania bazy (patrz naglowek rozbicie-stanu).
// Tu doklejamy im to, co siega do GT: `kandydaci` w RODZAJE_STREF i bazodanowy licznik do
// rozbijStanK4. Publiczne API gt-dokumenty (rozbijStanK4, RODZAJE_STREF, PRIORYTET_PRZYDZIALU,
// przydzielZwroty) zostaje bez zmian.
const rozbicieStanu = require('./rozbicie-stanu');
const { KUBELKI_STREF, PRIORYTET_PRZYDZIALU, przydzielZwroty } = rozbicieStanu;
...
// przydzielZwroty (PRZYDZIAL ILOSCIOWY zwrotow do rozmontowan) mieszka teraz w rozbicie-stanu -
// czysta funkcja, testowana bez GT/SQLite. Tu jest importowana (patrz gora pliku). Wywolania
// wstrzykuja realne okno KFS jako 3. argument (OKNO_ROZMONTOWANIE_KFS_DNI).
...
// kandydaci = zapytanie odwrotne ("ktore SKU maja w ogole taki dokument", gdy nie znamy zbioru
// z gory) - siega do GT, wiec zyje tu, nie w rozbicie-stanu. Doklejamy je do kanonicznej listy
// rodzajow (KUBELKI_STREF). Mapa laczy sie PO KUBELKI_STREF, wiec nowy rodzaj dodany tam MUSI
// dostac tu kandydata (inaczej jasny blad przy budowie modulu), a nie zniknie po cichu.
const KANDYDACI_STREF = {
  dostawa:        pobierzTowaryZDostawamiK4,
  zwrot:          pobierzTowaryZeZwrotamiK4,
  przywozka:      pobierzTowaryZPrzywozkamiK4,
  przyjecie_wewn: () => pobierzTowaryZPrzyjeciamiWewnK4(),
};
const RODZAJE_STREF = Object.fromEntries(
  Object.entries(KUBELKI_STREF).map(([rodzaj, kubelek]) => {
    const kandydaci = KANDYDACI_STREF[rodzaj];
    if (!kandydaci) throw new Error(`RODZAJE_STREF: rodzaj "${rodzaj}" bez funkcji kandydaci`);
    return [rodzaj, { kubelek, kandydaci }];
  })
);
...
// rozbijStanK4, kolejnosc zjadania (PRIORYTET_PRZYDZIALU) i cala arytmetyka kubelkow zyja teraz
// w rozbicie-stanu - czyste, testowane bez GT/SQLite (test/rozbij-stan-k4). Tu zostaje cienki
// wrapper, ktory wstrzykuje bazodanowy licznik "ile z dokumentu juz rozlozono"
// (iloscRozlozonaZDokumentu, siega do SQLite przez ruchy). Dzieki temu 9 istniejacych wywolan
// rozbijStanK4(stan, wms, dok, { artykul_gt_id, magazyn }) dziala bez zmian.
function rozbijStanK4(stanGt, sumaWms, dokumenty, opcje = {}) {
  return rozbicieStanu.rozbijStanK4(stanGt, sumaWms, dokumenty,
    { iloscRozlozona: iloscRozlozonaZDokumentu, ...opcje });
}
