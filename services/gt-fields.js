...
function obliczPolaLokalizacji(artykulGtId) {
  ...
  return {
    miejsce_na_magazynie: k4 ? k4.kod : '',
...
function zgodneZWms(artykulGtId, polaGt) {
  ...
  return (polaGt?.tw_Pole1 || '').trim() === oczekiwane.miejsce_na_magazynie
    ...
    && (polaGt?.pwd_Tekst09 || '').trim() === oczekiwane.lokalizacja_zapas;
}
...
const ZGODNOSC = { NIEZGODNE: 'NZ', TYLKO_GT: 't_GT', ZGODNE: 'OK', PUSTE: 'BD' };
// kolejnosc "najgorszy przypadek" - do wyznaczenia jednej wartosci na wiersz z K4+K4G
const PRIORYTET_ZGODNOSCI = [ZGODNOSC.NIEZGODNE, ZGODNOSC.TYLKO_GT, ZGODNOSC.ZGODNE, ZGODNOSC.PUSTE];
...
async function pobierzPrzegladLokalizacji(artykulGtIds) {
  ...
  for (const id of new Set(artykulGtIds.map(String))) {
    ...
    const oczekiwane = obliczPolaLokalizacji(id);

    const gtK4 = (polaGt?.tw_Pole1 || '').trim();
    const gtK4g = [(polaGt?.tw_Pole8 || '').trim(), (polaGt?.pwd_Tekst09 || '').trim()].filter(Boolean).join('; ');
    const wmsK4g = [oczekiwane.lokalizacja_gorna, oczekiwane.lokalizacja_zapas].filter(Boolean).join('; ');

    const k4 = { gt_tekst: gtK4, stan: klasyfikujZgodnosc(oczekiwane.miejsce_na_magazynie, gtK4) };
    const k4g = { gt_tekst: gtK4g, stan: klasyfikujZgodnosc(wmsK4g, gtK4g) };
